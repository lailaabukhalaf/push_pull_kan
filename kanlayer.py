...
Original file is located at
    https://colab.research.google.com/drive/10OGoBDwXKsYU6fHQNcdoGUjEiScZzkGa
"""

# 1) define how many layers we want 2-layer KAN model
class IrisKAN(nn.Module):
    def __init__(self):
        super().__init__()
        self.kan1 = KANLayer(in_dim=4, out_dim=8, num=5, k=3, grid_range=[-1,1])
        self.kan2 = KANLayer(in_dim=8, out_dim=3,  num=5, k=3, grid_range=[-1,1])
    def forward(self, x):
        x, *_ = self.kan1(x)
        x, *_ = self.kan2(x)
        return x
...
# creating basis function (cox de boor formula recursive function )
...
def curve2coef(x_eval, y_eval, grid, k):
    ...
    --------
        coef : 3D torch.tensor
            shape (in_dim, out_dim, G+k-1). original stated (g+k)  test what is going to be cause it must be (g+k-1)
    '''
    ...
    try:
        coef = torch.linalg.lstsq(mat, y_eval).solution[:,:,:,0] # least square
    except:
        print('lstsq failed')
...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.5, scale_base_mu=0.0, scale_base_sigma=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, save_plot_data = True, device='cpu', sparse_init=False):
        ...
        self.coef = torch.nn.Parameter(curve2coef(self.grid[:,k:-k].permute(1,0), noises, self.grid, k)) # we define the coef as trained parameter be define from curve2coef
        ...
        self.scale_sp = torch.nn.Parameter(torch.ones(in_dim, out_dim) * scale_sp * 1 / np.sqrt(in_dim) * self.mask).requires_grad_(sp_trainable)  # make scale trainable scale spline wich is the w2
    ...
    def forward(self, x):
        ...
        base = self.base_fun(x) # (batch, in_dim) comput the base
        y = coef2curve(x_eval=x, grid=self.grid, coef=self.coef, k=self.k) # compute the activation spline
        ...
        postacts = y.clone().permute(0,2,1) # save y after masking
